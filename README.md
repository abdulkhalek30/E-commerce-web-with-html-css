<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Panda E-commerce</title>
    <link rel="stylesheet" href="index.css">
</head>
<body>
    <header>
        <div class="logo"> <img src="logo.png" alt=""></div>
        <nav>
            <ul>
                <li><a href="#">Home</a></li>
                <li><a href="#">Product</a></li>
                <li><a href="#">About Us</a></li>
                <li><a href="#">Contact Us</a></li>
            </ul>
        </nav>
    </header>
    <section class="hero">
        <div class="hero-content">
            <h1>X-Box for your living room </h1>
            <p>This is the best x-box in the world for people who just want to waste time in front of fake  sports</p>
            <p class="price">$600</p>
            <button>Buy Now ➡</button>
        </div>
        <img src="xbox.png" alt="">
    </section>
    <section class="categories">
        <button>Watch</button>
        <button>Bag</button>
        <button>Shoes</button>
    </section>
    <section class="products-1">
        <h2>Shoes</h2>
        <div class="product-grid">
            <div class="product">
                <img src="shoe-1.png" alt="Shoe">
                <h3>Reebok Dart Men's Shoes</h3>
                <p>Amet minim mollit non deserunt ullamco est sit aliqua dolor do amet sint.</p>
                <p class="price">$120</p>
                <button>Buy Now ➡</button>
            </div>
            <div class="product">
                <img src="shoe-2.png" alt="Shoe">
                <h3>Reebok Dart Men's shoes</h3>
                <p>Amet minim mollit non deserunt ullamco est sit aliqua dolor do amet sint.</p>
                <p class="price">$200</p>
                <button>Buy Now ➡</button>
            </div>
            <div class="product">
                <img src="shoe-3.png" alt="Shoe">
                <h3>Reebok Dart Men's shoes</h3>
                <p>Amet minim mollit non deserunt ullamco est sit aliqua dolor do amet sint.</p>
                <p class="price">$300</p>
                <button>Buy Now ➡</button>
            </div>
        </div>
    </section>
    <section class="products-2">
        <h2>Backpack</h2>
        <div class="product-grid">
            <div class="product">
                <img src="bag-1.png" alt="back">
                <h3>Red Laltu Bag</h3>
                <p>Amet minim mollit non deserunt ullamco est sit aliqua dolor do amet sint.</p>
                <p class="price">$100</p>
                <button>Buy Now ➡</button>
            </div>
            <div class="product">
                <img src="bag-2.png" alt="back">
                <h3>Reebok Dart Men's BAg</h3>
                <p>Amet minim mollit non deserunt ullamco est sit aliqua dolor do amet sint.</p>
                <p class="price">$130</p>
                <button>Buy Now ➡</button>
            </div>
            <div class="product">
                <img src="bag-3.png" alt="back">
                <h3>Reebok Dart Men's Bag</h3>
                <p>Amet minim mollit non deserunt ullamco est sit aliqua dolor do amet sint.</p>
                <p class="price">$120</p>
                <button>Buy Now ➡</button>
            </div>
        </div>
    </section>
    <section class="newsletter">
        <h2>LET'S STAY IN TOUCH</h2>
        <p>Get updates on sales specials and more</p>
        <input type="email" placeholder="Email">
        <button>SUBMIT</button>
    </section>
</body>
</html>
